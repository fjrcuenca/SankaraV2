...
/** Could possibly remove school/camp/anganwadi name!! **/
	...
	function get_screening_sites() {
		$qry = mysql_query('SELECT * from screening_site');
		$data = array();
	...
	function get_screening_site() {
		...
		$data = array();
		if ($kind == 'School') {
			$qry = mysql_query("SELECT * from screening_site where school_name = '$name'");
			while($rows = mysql_fetch_array($qry))
			{
				$data[] = array(
					"school_name"	=>	$rows['school_name'],
			...
			$qry = mysql_query("SELECT * from screening_site where camp_name = '$name'");
			while($rows = mysql_fetch_array($qry))
			{
				$data[] = array(
					"camp_name"	=>	$rows['camp_name'],
			...
			$qry = mysql_query("SELECT * from screening_site where anganwadi_name = '$name'");
			while($rows = mysql_fetch_array($qry))
			{
				$data[] = array(
					"anganwadi_name"	=>	$rows['anganwadi_name'],
	...
	function add_school() {
		...
		print_r($data);
		$qry = 'INSERT INTO screening_site 
	...
	function get_schools() {
		...
		$qry = mysql_query('SELECT * from screening_site where school_name is not NULL');
		$data = array();
		while($rows = mysql_fetch_array($qry))
		{
			$data[] = array(
...
/********** END ADDED ********************/	
	...
	function get_school() {
		$school = json_decode(file_get_contents("php://input"));
